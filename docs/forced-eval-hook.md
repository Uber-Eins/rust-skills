...
> 解决 Codex Skills 自动触发不可靠的问题

## 问题背景

### Codex 的行为特点

> "Codex is so goal focused that it barrels ahead with what it thinks is the best approach. It doesn't check for tools unless explicitly told to."

Codex 非常专注于目标，会直接用它认为最好的方式处理问题，**不会主动检查可用的 skills**，即使 skill 的 description 中有匹配的关键词。

### 触发成功率对比

| 方法 | 成功率 | 说明 |
|------|--------|------|
| 仅靠 description 关键词 | **~20%** | Codex 很少主动检查 |
| 简单指令 hook | 40-50% | "建议"检查，Codex 可能忽略 |
...
**强制** Codex 在执行任务前必须评估每个 skill，而不是"建议"它检查。
...
2. **列出所有 skills 及其关键词**: 减少 Codex 遗漏
...
Hook 注入的文本使用了类似"系统指令"的语气，Codex 会将其视为高优先级指令。

### 2. 明确的检查清单

列出所有 skills 和关键词，让 Codex 知道有哪些选项可用，而不是依赖它自己去发现。

### 3. 强制输出推理过程

要求输出 "YES/NO - reason" 格式，强制 Codex 进行显式推理，而不是直接跳过。

### 4. 延迟执行

"Only THEN proceed" 确保 Codex 先完成评估和激活，再开始实际回答。
...
**问题**: Codex 不会主动检查 skill descriptions
...
- [Scott Spence: Codex Skill Auto Activation](https://scottspence.com/posts/codex-skill-auto-activation)
- [Scott Spence: Codex Skill Auto Activation Follow Up](https://scottspence.com/posts/codex-skill-auto-activation-follow-up)
- [Codex Hooks Documentation](https://docs.openai.com/codex/hooks)
