...
## 1. CSO (Codex Search Optimization) - 描述优化

### 问题
Skills 的 `description` 字段是 Codex 匹配用户问题的关键，但很多 skill 描述不够优化，导致无法被正确触发。
...
```
用户问题 → Codex 匹配所有 skills 的 description
         → 多个 skills 可能同时触发
...
动态生成的 crate skills 直接放在 `~/.codex/skills/` 下，Codex 自动扫描：
...
| 符号链接 | 动态 skills 需要链接到 ~/.codex/skills/ |
| 文档检查 | 读取失败时提示用户更新 |
