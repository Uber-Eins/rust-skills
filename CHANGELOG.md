...
### Added
- Integrated `leonardomso/rust-skills` as `rust-best-practices`, a 265-rule
  idiomatic Rust rule library with upstream MIT license attribution.
- Added a Codex agent manifest for best-practice rule lookup.
- Added lightweight rule index validation under
  `skills/rust-best-practices/checks/validate.py`.

### Changed
- Routed Rust code review, idiomatic Rust, and best-practice requests through
  `rust-best-practices` alongside `coding-guidelines`.
- Updated the Rust-only hook prompt to load detailed best-practice rules for
