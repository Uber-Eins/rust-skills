...
Skills には**インラインフォールバックロジック**が組み込まれており、エージェントファイルが利用できない場合、組み込みツール（actionbook、chrome-devtools、WebFetch）を使用して直接実行します。
...
| **chrome-devtools-mcp** | JavaScript 対応ページ自動化と抽出のための Chrome DevTools MCP サーバー。actionbook セレクターが使えない場合のフォールバック。 | [ChromeDevTools/chrome-devtools-mcp](https://github.com/ChromeDevTools/chrome-devtools-mcp) |
...
- [@pinghe](https://github.com/pinghe) - `context: fork` サポートの提案 ([#4](https://github.com/Uber-Eins/rust-skills/issues/4))
- [@DoiiarX](https://github.com/DoiiarX) - OpenCode インストール修正 ([#6](https://github.com/Uber-Eins/rust-skills/issues/6))
